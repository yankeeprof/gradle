...
The `libsDir` and `distsDir` properties were removed from the `BasePluginConvention` class. Clients should replace usages with the `libsDirectory` and `distsDirectory` properties.
Also, `DefaultBasePluginConvention` no longer offers a reference to the current project.
